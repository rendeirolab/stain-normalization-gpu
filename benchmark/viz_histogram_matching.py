...
def visualize_histogram_matching():
    ...
    target_cp = cp.array(target_img).transpose(2, 0, 1)[
        None, ...
    ]  # Add batch dimension
    test_images_cp = [
    ...
    # Ensure all images are in [0, 1] range for display
    target_np = (
        np.clip(target_np / 255.0, 0, 1)
        if target_np.max() > 1
        else np.clip(target_np, 0, 1)
    )
    test_images_np = [
        np.clip(img / 255.0, 0, 1) if img.max() > 1 else np.clip(img, 0, 1)
